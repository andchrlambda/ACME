...
As always, make sure to managgit ste your time - get a section/question to "good
...
Code review should ensure that code is formatted and commented so that the author and others can easily understand the code's purpose and strategy. PEP8 is the industry standard baseline for formatting and commenting in Python. Unit tests ensure that updates and refactoring do not have unintended consequences or break the existing code. Implementing unit tests in projects, especially large ones, will save time and reduce frustration.

Containers such as Docker create reproducible builds across operating systems. Dockerfiles specify a base and all dependencies to automate the installation and configuration of an image. Containers are ideal for project teams that are working on different platforms. Containers are more exciting than they sound. Just think of all the goodies hidden from view in shipping containers.
